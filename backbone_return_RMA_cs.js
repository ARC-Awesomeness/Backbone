...
define(['N/search'], function (search) {
    ...
    function fieldChanged(scriptContext) {
        ...
        try {
            ...
            if (!scriptContext.sublistId && fieldId === 'custbody_cbiz_back_ret_type') {
                ...
                for (var line = 0; line < lineCount; line++) {
                    ...
                    CurrentRecord.setCurrentSublistValue({ sublistId: sublistId, fieldId: 'custcol_cbiz_bac_ret_typ', value: CurrentRecord.getValue({ fieldId: fieldId }), ignoreFieldChange: false });
    ...
    function postSourcing(scriptContext) {
        ...
        try {
            ...
            if (sublistId === 'item' && fieldId === 'item' && CurrentRecord.getCurrentSublistValue({ sublistId: sublistId, fieldId: fieldId })) {
                log.debug('in postsource ');

                CurrentRecord.setCurrentSublistValue({ sublistId: sublistId, fieldId: 'custcol_cbiz_bac_ret_typ', value: CurrentRecord.getValue({ fieldId: 'custbody_cbiz_back_ret_type' }), ignoreFieldChange: false });
            }
        } catch (err) {
